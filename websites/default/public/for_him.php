...
$query = "SELECT * FROM products WHERE category = gender = 'M' ORDER BY {$sortingOptions[$sort]} LIMIT :limit OFFSET :offset";
...
$totalProductsQuery = "SELECT COUNT(*) as total FROM products WHERE category = gender AND gender = 'M'";
...
<main>
    ...
    <link rel="stylesheet" href="core/styles.css">
        ...
        <select id="sort" name="sort" onchange="this.options[this.selectedIndex].value && (window.location = 'for_him.php?sort=' + this.options[this.selectedIndex].value)">
    ...
    <div class="pagination">
        <?php if ($totalPages > 1) : ?>
            <?php for ($i = 1; $i <= $totalPages; $i++) : ?>
                <a href="for_him.php?sort=<?php echo $sort; ?>&page=<?php echo $i; ?>" <?php echo ($page === $i) ? 'class="active"' : ''; ?>><?php echo $i; ?></a>
            <?php endfor; ?>
        <?php endif; ?>
    </div>
</main>
